...
int SimonDemo::score = 0;

SimonDemo::SimonDemo(int hp, int damage, int point) : Enemy(hp, damage, point)
{
	respawnTime = 0;
	respawnTimeNeo = 0;
}
...
bool SimonDemo::Initialize(LPCWSTR texturePath, float x, float y, int tag, int id)
{
	...
	whip = new WhipDemo();
	if (!whip->Initialize( WHIP_SPRITE, 0, 0, TAG_WHIP, 777))
		return false;
...
void SimonDemo::Reset(D3DXVECTOR2 pos)
{
	isLeft = false;
	isGrounded = false;
	velocity.x = 0;
...
void SimonDemo::OnStairHandle(float gameTime)
{
	if (isCollideWith25)
	{
		if (IsKeyDown(DIK_UP))
...
void SimonDemo::SetInvincible(int time)
{
	invincibleTime = time;
	isInvincible = true;
}

void SimonDemo::CreateAnimation()
{
	anim = new AnimationDemo();
	anim->Initialize(sprite, 60.0f, 66.0f, 8, 4, (int)Type::SIMON_EDIT, 0.15f);
...
void SimonDemo::Update(float gameTime)
{
	SetRegion(0, width, 0, -height);
...
void SimonDemo::Render(ViewPort* viewPort)
{
	if (!isSplashing)
	{
...
D3DXVECTOR2 SimonDemo::GetVelocity()
{
	return velocity;
}

void SimonDemo::UpdateKeyboard(float gameTime)
{
...
void SimonDemo::Fighting()
{
	...
	{
		if (action == HIT_SIT)
			whip->SetPosition(D3DXVECTOR2(this->GetPosition().x, this->GetPosition().y), false);
		else
			whip->SetPosition(D3DXVECTOR2(this->GetPosition().x, this->GetPosition().y), true);
...
void SimonDemo::StandOnStair()
{
	velocity.x = 0;
	velocity.y = 0;
}

void SimonDemo::KeyBoardHandle(float gameTime)
{
...
void SimonDemo::CheckCollider(float gameTime, std::vector<GameObjectDem*>* listGameObject)
{
...
void SimonDemo::CheckCollideWithGround(float gameTime, std::vector<GameObjectDem*>* listGameObject)
{
	float normalX = 0;
	float normalY = 0;
...
void SimonDemo::CheckCollideWithGround(float gameTime, GameObjectDem* gameObject)
{
	float normalX = 0;
	float normalY = 0;
	float timeCollide;
...
void SimonDemo::CheckCollideWithStair(float gameTime, std::vector<GameObjectDem*>* listGameObject)
{
	float normalX = 0;
	float normalY = 0;
	float timeCollide;
...
void SimonDemo::CheckCollideWithWall(float gameTime, std::vector<GameObjectDem*>* listGameObject)
{
	float normalX = 0;
	float normalY = 0;
	float timeCollide;
...
void SimonDemo::CheckCollideWithEnemy(float gameTime, std::vector<GameObjectDem*>* listGameObject)
{
	{
		float normalX = 0;
		float normalY = 0;
...
void SimonDemo::CheckColliderWithMovableGround(float gameTime, std::vector<GameObjectDem*>* listGameObject)
{
	float normalX = 0;
	float normalY = 0;
	float timeCollide;
...
bool SimonDemo::IsCollideWith(float gameTime, GameObjectDem* object)
{
	return Enemy::IsCollideWith(gameTime, object);
}

void SimonDemo::CheckColliderWith(float gameTime, GameObjectDem* object)
{
	float normalX = 0;
	float normalY = 0;
	float timeCollide;
...
void SimonDemo::WhipCheckCollider(float gameTime, std::vector<GameObjectDem*>* listGameObject)
{
...
/*void SimonDemo::CheckColliderWithDoor(float gameTime, std::vector<GameObjectDem*>* listGameObject)
{
	float normalX = 0;
	float normalY = 0;
	float timeCollide;
...
void SimonDemo::CollideWithDoorHandle(float gameTime, std::vector<GameObjectDem*>* listGameObject, ViewPort* viewPort)
{
...
bool SimonDemo::IsColliderWithCheckPoint(float gameTime, std::vector<GameObjectDem*>* listGameObject)
{
	float normalX = 0;
	float normalY = 0;
	float timeCollide;
...
void SimonDemo::CheckCollieWithTopStair(float gameTime, std::vector<GameObjectDem*>* listGameObject)
{
	float normalX = 0;
	float normalY = 0;
	float timeCollide;
...
void SimonDemo::ChangeSenceStairCheck(float gameTime, std::vector<GameObjectDem*>* listGameObject, ViewPort* viewPort)
{
	float normalX = 0;
	float normalY = 0;
	float timeCollide;
...
/*void SimonDemo::SetTypeOfSubWeapon(int subType)
{
	for (int i = 0; i < 3; i++)
	{
		switch (subType)
