...
bool Enemy::Initialize(LPCWSTR texturePath, float x, float y, int tag, int id)
{
	isMoveable = true;
	isIn = false;
	...
	return GameObjectDem::Initialize(texturePath, x, y, tag, id);
}
...
{
	GameObjectDem::SetBox(sprite->GetPosition().x - offsetX,
...
void Enemy::CheckCollider(float gameTime, std::vector<GameObjectDem*>* listGameObject)
{
	float normalX = 0;
	float normalY = 0;
	float timeCollide;
...
bool Enemy::IsCollideWith(float gameTime, GameObjectDem* object)
{
	float normalX = 0;
	float normalY = 0;
	float timeCollide;
