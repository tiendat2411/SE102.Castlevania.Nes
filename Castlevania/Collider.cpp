...
std::vector<CCollider*> CCollider::listCollider;
...
Box CCollider::GetSweptBoardphaseBox(Box box, float gameTime)
{
	Box broadphasebox;
...
bool CCollider::AABBCheck(Box b1, Box b2)
{
	return !(b1.right < b2.left || b1.left > b2.right || b1.top < b2.bottom || b1.bottom > b2.top);
}

float CCollider::SweptAABB(float gameTime, Box b1, Box b2, float& normalX, float& normalY)
{
...
Box CCollider::GetBox()
{
	return box;
}

void CCollider::SetBox(Box _box)
{
	box = _box;
}

int CCollider::GetTag()
{
	return tag;
}

void CCollider::SetTag(int _tag)
{
	tag = _tag;
}
